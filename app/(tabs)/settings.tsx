...
type Settings = {
  ...
  middayHour: number;        // Local hour for the midday check-in (default 1pm)
...
async function persistAndSync(settings: Settings, userId: string) {
  ...
  const writes: [string, string][] = [
    ...
    ['middayHour', String(settings.middayHour)],
  ...
  fetch(`${API_BASE}/signals?type=preferences`, {
    ...
    body: JSON.stringify({
      ...
      preferences: {
        ...
        middayHour: settings.middayHour,
...
async function loadSettings(): Promise<Settings> {
  ...
  try {
    ...
    return {
      ...
      middayHour: (() => {
        const n = parseInt(map.middayHour || '', 10);
        return isNaN(n) ? DEFAULTS.middayHour : n;
      })(),
      overwatchHour: (() => {
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <ToggleRow
          ...
          subtext="A quick afternoon status check — what's resolved, what's left"
        ...
        {settings.middayEnabled ? (
          <ChevronRow
            label="Midday time"
            rightText={format12Hour(`${String(settings.middayHour).padStart(2, '0')}:00`)}
            onPress={() => {
              const opts = [11, 12, 13, 14, 15];
              Alert.alert('Midday check-in time', undefined, [
                ...opts.map((h) => ({
                  text:
                    format12Hour(`${String(h).padStart(2, '0')}:00`) +
                    (settings.middayHour === h ? '  ✓' : ''),
                  onPress: () => update({ ...settings, middayHour: h }),
                })),
                { text: 'Cancel', style: 'cancel' as const },
              ]);
            }}
